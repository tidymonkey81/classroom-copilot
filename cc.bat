...
if "%choice%"=="2a" (
    ...
    start cmd /k "cd backend && conda activate cc && uvicorn routers.main:app --host 0.0.0.0 --port 9500"
    goto host_menu
)
if "%choice%"=="2b" (
    start cmd /k "cd frontend && npm run dev"
...
if "%choice%"=="2c" (
    start cmd /k "cd backend\app && uvicorn routers.main:app --host 0.0.0.0 --port 9500"
    goto host_menu
)
if "%choice%"=="2d" (
